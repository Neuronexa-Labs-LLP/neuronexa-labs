    ...
    <section id="how-it-works" className="py-20 md:py-24 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          <span className="inline-block text-[#2AA7D3] text-sm font-semibold tracking-widest uppercase mb-4">Our Process</span>
        ...
        <div className="hidden lg:grid grid-cols-5 gap-0 relative">
          {/* Connector Line */}
          <div className="absolute top-7 left-[10%] right-[10%] h-[2px] bg-gradient-to-r from-[#2AA7D3]/20 via-[#2AA7D3]/40 to-[#2AA7D3]/20"></div>

          {steps.map((step, index) => (
            <div key={step.title} className="flex flex-col items-center text-center px-4 relative group">
              {/* Numbered Circle */}
              <div className="w-14 h-14 rounded-full bg-white border-2 border-[#2AA7D3]/30 flex items-center justify-center mb-6 relative z-10 group-hover:border-[#2AA7D3] group-hover:shadow-[0_0_20px_rgba(42,167,211,0.2)] transition-all duration-300">
                <step.icon className="w-6 h-6 text-[#0F6F94] group-hover:text-[#2AA7D3] transition-colors duration-300" />
              </div>

              {/* Step Label */}
              <span className="text-xs font-bold text-[#2AA7D3] tracking-wider uppercase mb-2">Step {index + 1}</span>
          ...
          <div className="flex flex-col gap-8">
            {steps.map((step, index) => (
              <div key={step.title} className="relative">
                {/* Circle on the line */}
                <div className="absolute -left-14 top-0 w-12 h-12 rounded-full bg-white border-2 border-[#2AA7D3]/30 flex items-center justify-center z-10">
                  <step.icon className="w-5 h-5 text-[#0F6F94]" />
                </div>
                {/* Connector line to next step — hidden on last step */}
                {index < steps.length - 1 && (
                  <div className="absolute -left-[33px] top-12 w-[2px] h-[calc(100%+2rem)] bg-[#2AA7D3]/20"></div>
                )}
                {/* Text content */}
                <div className="pt-1">
                  <span className="text-xs font-bold text-[#2AA7D3] tracking-wider uppercase">Step {index + 1}</span>
