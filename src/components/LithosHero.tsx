...
const RevealLayer = ({ cursorX, cursorY }: { cursorX: number, cursorY: number }) => {
  ...
  useEffect(() => {
    ...
    // Only draw if we have a valid cursor
...
export const LithosHero: React.FC = () => {
  ...
  return (
    <div className="w-full tracking-[-0.02em] font-sans" style={{ fontFamily: "'Inter', sans-serif" }}>
      <nav className="absolute top-0 left-0 right-0 z-[100] flex items-center justify-between p-4 sm:p-5">
        ...
        <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 bg-white/20 backdrop-blur-md border border-white/30 rounded-full px-2 py-2 items-center gap-1">
          ...
          {['Field Guides', 'Geology', 'Plans', 'Live Tour'].map(item => (
             <button key={item} className="text-white/80 px-4 py-1.5 rounded-full text-sm font-medium hover:bg-white/20 hover:text-white transition-colors">{item}</button>
          ))}
        </div>

        <button className="hidden md:block bg-white text-gray-900 text-sm font-semibold px-6 py-2.5 rounded-full hover:bg-gray-100 transition-colors cursor-pointer">
          Sign Up
        </button>
      </nav>
